cREATE or alter view [dbo].[VW_ACD_VALIDACAO_NOTAS] as 
...
group by curso.nome, cur.nome, aluno.nome, disc.nome, etapa.nome, status.nome, statusdisc.nome,ca.id, disc.id,
         grade.id, cur.id, gequ.curriculo_id, turma.id, tda.id, td.id,turma.etapa_ano_id 
...
select *,tur.nome as turma_nome from (
			select distinct nta.aluno, equ.disciplina, equ.disciplina_equivalente ,nta.SituacaoDisciplina, nta.StatusCurricular,
				   isnull(rec.nota_final_recuperacao, nta.nota) as NOTA_FINAL,   rec.nota_recuperacao, rec.nota_final_recuperacao,
			       dsc.nota as nota_historico, nta.turma_id, nta.disciplina_id, nta.turma_disciplina_id, nta.turma_disciplina_aluno_id,
				   sdc.nome as status_disciplina_concluida_nome
			  from cte_notas nta 
                                 ...
                                 join curriculos_statusdisciplina    sdc on (sdc.id = dsc.status_id)
			...
			select distinct nta.aluno, nta.disciplina, disciplina_equivalente = null, nta.SituacaoDisciplina, nta.StatusCurricular,
			  ...
			  from cte_notas nta join curriculos_disciplinaconcluida dsc on (dsc.curriculo_aluno_id = nta.curriculo_aluno_id and
																			 dsc.disciplina_id = nta.disciplina_id and 
																			 nta.etapa_ano_id = dsc.etapa_ano_id)
                                 join curriculos_statusdisciplina    sdc on (sdc.id = dsc.status_id)
							left join cte_recuperacao                rec on (nta.curriculo_aluno_id = rec.curriculo_aluno_id and
			...
			WHERE  
				  isnull(rec.nota_final_recuperacao, nta.nota) <> dsc.nota and 
				  nta.curriculo_id = nta.curriculo_id_equ
			) as tab join academico_turma tur on (tur.id = tab.turma_id)
