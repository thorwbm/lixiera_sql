...
-- #### recuperar o ra do aluno

	select @aluno_ra = ISNULL(aluno_ra,'VAZIO') 
      from vw_Curriculo_aluno_pessoa 
     where curriculo_aluno_id = @curriculo_aluno_id AND 
	       curriculo_aluno_status_id = 13 -- STATUS CURSANDO
...
select distinct    
    ...
    vigente                   = 1,     
    tipo_id                   = 1,     
    DESCRICAO                 = 'Contrato de Matrícula CMMG'      
  from     
       academico_aluno alu   JOIN VW_FNC_PARCELA_MAT_REM        par on (ALU.ra collate database_default          = PAR.ra collate database_default)      
							 ...
							 JOIN CURRICULOS_ALUNO              CRA ON (CUR.ID = CRA.curriculo_id AND ALU.ID = CRA.ALUNO_ID) 
						...
						left join contratos_contrato            xxx on (xxx.aluno_id = cra.aluno_id and 
						                                                xxx.curriculo_id = cra.curriculo_id and 
																		xxx.vigente = 1 and xxx.descricao = 'Contrato de Matrícula CMMG') 
  where         
	   ...
	   UPDATE CON SET VIGENTE = 0
	     FROM CONTRATOS_CONTRATO CON JOIN vw_Curriculo_aluno_pessoa CAP ON (CAP.aluno_id = CON.aluno_id AND 
		                                                                    CAP.curriculo_id = CON.curriculo_id)	 
	   WHERE CAP.curriculo_aluno_id = @curriculo_aluno_id AND 
	         CON.DESCRICAO = 'Contrato de Matrícula' AND 
			 CON.VIGENTE = 1
...
select criado_em       = @DATAEXECUCAO, criado_por     = @USUARIO,      
  ...
  from     
       contratos_contrato con join vw_Curriculo_aluno_pessoa pes on (con.aluno_id     = pes.aluno_id and       
                                                                     con.curriculo_id = pes.curriculo_id)      
						 ...
						 left join contratos_parcela         xxx on (xxx.contrato_id     = con.id and      
                                                                     xxx.mes_competencia = par.mes_competencia and      
                                                                     xxx.ano_competencia = par.ano_competencia)
 where    
       CON.VIGENTE = 1 AND
	   CON.TIPO_ID = 1 AND
	   PAR.RA = @ALUNO_RA  AND
...
select        
  ...
  from         
       contratos_contrato con join vw_Curriculo_aluno_pessoa pes on (con.aluno_id = pes.aluno_id and       
                              ...
                              join DESCONTO_PORTAL_MATRICULA dpd on (dpd.desconto_matricula collate database_default = des.desconto_tipo collate database_default )      
                              join contratos_tipodesconto    tpd on (tpd.descricao = dpd.DESCONTO_PORTAL)   
						 left join contratos_desconto        xxx on (xxx.parcela_id = par.id and
						                                             xxx.tipo_id    = tpd.id and
																	 xxx.valor      = des.valor)
 where 
       con.vigente = 1 and
	   con.tipo_id = 1 and
...
select distinct       
 ...
 where   
       con.tipo_id = 1 and 
	   con.vigente = 1 and 
       cra.curriculo_aluno_id = @curriculo_aluno_id and
       xxx.id is null     
 group by       
 ...
 select criado_em = @DATAEXECUCAO, atualizado_em = @DATAEXECUCAO, criado_por = @USUARIO, atualizado_por = @USUARIO,      
  ...
  from       
        contratos_desconto ctd join contratos_parcela         par on (par.id = ctd.parcela_id)  
							   ...
							   join vw_Curriculo_aluno_pessoa cra on (cra.aluno_id = con.aluno_id and
							                                          cra.curriculo_id = con.curriculo_id)		                       
                               join financeiro_lancamento     lan on (par.id = lan.parcela_id and 
							                                          con.id = lan.contrato_id)      
...
set @passo = 'INSERIR FINANCEIRO BOLETO'      
 ...
 select       
  ...
  where 
	...
	group by PAR.BOLETO_ID, LAN.ID, LAN.pago_em, LAN.VALOR_PAGO, LAN.valor_acrescimos,LAN.BANCO, 
	         LAN.NEGOCIACAO_ID,  lan.valor_desconto_pontualidade
...
set @passo = 'INSERIR NEGOCIACAO'      
 ...
 INSERT INTO       
       FINANCEIRO_NEGOCIACAO (descricao, valor, conta_id, responsavel_id, criado_em, atualizado_em, criado_por, atualizado_por, autorizado_por_id)          
select distinct      
  ...
  from       
       financeiro_lancamento lan join contratos_contrato        con on (con.id = lan.contrato_id)      
					        ...
					        left join financeiro_negociacao     xxx on (xxx.descricao = 'Pagamento boleto - ' +  convert(varchar(10),par.boleto_id))
 where 
       con.vigente = 1 and 
	   con.tipo_id = 1 and 
	   pes.curriculo_aluno_id = @curriculo_aluno_id and
