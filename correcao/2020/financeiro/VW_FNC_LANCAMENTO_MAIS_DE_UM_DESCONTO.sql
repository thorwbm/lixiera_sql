...
with cte_lancamento_mais_um_desconto as (
			select lan.id as lancamento_id, des.id as desconto_id
			  from 
				   financeiro_lancamento lan join financeiro_desconto des on (lan.id = des.lancamento_id)
) 
	,	cte_mais_de_um as (
			select lancamento_id 
			  from cte_lancamento_mais_um_desconto
             group by lancamento_id 
			having count(desconto_id) > 1
)
			...
			select crc.id as curriculo_id, crc.nome as curriculo_nome,
				   ...
				   con.id as contrato_id, con.criado_por as contrato_criado_por, con.criado_em as contrato_criado_em
...
go
---------------------------------------------------------------------------------------------------------
       select * from vw_fnc_lancamento_mais_de_um_desconto
			 order by curriculo_nome, aluno_nome, lancamento_id, lancamento_ano, lancamento_mes, desconto_tipo_nome
