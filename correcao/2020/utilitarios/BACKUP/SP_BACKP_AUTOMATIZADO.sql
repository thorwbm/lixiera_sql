...
DECLARE @DATABASE VARCHAR(200), @SQL VARCHAR(MAX), @SQL_DATA VARCHAR(50)
SET @SQL_DATA = REPLACE(REPLACE(CONVERT(VARCHAR(19), GETDATE(), 121),':','_'), ' ','__')
...
declare CUR_ cursor for 
	...
	open CUR_  
		...
		while @@FETCH_STATUS = 0
			BEGIN
				SET @SQL = N'EXEC msdb.dbo.rds_backup_database
							@source_db_name = ' + CHAR(39) +  @DATABASE + CHAR(39) +',
							@s3_arn_to_backup_to = '+ CHAR(39) +'arn:aws:s3:::educat-feluma-rds-backups/PRD/' + @DATABASE + '/'++ @DATABASE + '_'+ @SQL_DATA + '.bak'+ CHAR(39) +',
