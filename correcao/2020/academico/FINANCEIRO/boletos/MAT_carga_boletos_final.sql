...
--------------------------------------------------------------------------------------------------------------------
--select * from contratos_contrato order by id desc

set @passo = 'contrato'
insert into contratos_contrato (criado_em, atualizado_em, valor, valor_parcela, num_parcelas, num_fiadores, aluno_id, criado_por, curriculo_id, 
            etapa_ano_id, responsavel_financeiro_id, atualizado_por, data_matricula, vigente, tipo_id, descricao)
select distinct criado_em = getdate(), 
                atualizado_em = getdate(), 
				valor = -1, valor_parcela = -1, 
				num_parcelas = -1, 
				num_fiadores = ISNULL(fia.qtd_fiadores,0), 
                aluno_id = alu.id, criado_por = 2137, 
				curriculo_id = CUR.id, etapa_ano_id= null, 
				responsavel_financeiro_id = isnull(pes.id, alu.pessoa_id), 
				atualizado_por = 2137,
				data_matricula = par.data_matricula, 1,1, 'Contrato de Matrícula'
...
--------------------------------------------------------------------------------------------------------------------
 ...
 insert into financeiro_conta (criado_em, atualizado_em, saldo, criado_por, pessoa_id, atualizado_por, titular_id)
	   select criado_em = getdate(), atualizado_em = getdate(), saldo = 0.0, criado_por = 2137, 
	          pessoa_id = alu.pessoa_id , atualizado_por = 2137, titular_id = ALU.PESSOA_ID  
...
select descricao = 'desconto ' + DES.COMPETENCIA, porcentagem = null, valor = des.valor, parcela_id = par.id, 
       ...
       criado_em = getdate(), criado_por = 2137, 
	   atualizado_em = getdate(), atualizado_por = 2137
from contratos_contrato con join academico_aluno            alu on (con.aluno_id = alu.id)
...
select distinct 
	   ...
	   criado_por = 2137, curriculo_aluno_id = cra.id, operacao_id = 2, parcela_id = par.id, protocolo_id= null , 
	   status_id = case when PRX.status_parcela = 'PAGO' THEN 5 ELSE 1 END , 
	   tipo_id = case when Par.mes_competencia = 1 then 1 else 2 end, 
	   atualizado_por = 2137, extra = null, responsavel_id = con.responsavel_financeiro_id, valor_pago = isnull(prx.valor_pago,0), valor_acrescimos=0,valor_desconto_pontualidade =0
