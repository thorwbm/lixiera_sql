...
group by curso.nome, cur.nome, aluno.nome, disc.nome, etapa.nome, status.nome, statusdisc.nome,ca.id, disc.id,
         grade.id, cur.id, gequ.curriculo_id 
...
select * from (
select distinct nta.aluno, equ.disciplina, equ.disciplina_equivalente ,
       isnull(rec.nota_final_recuperacao, nta.nota) as NOTA_FINAL,   rec.nota_recuperacao, rec.nota_final_recuperacao,
   seperador = 'XXXXXXXXXXXXXXX',    dsc.nota as nota_historico
  from cte_notas nta 
...
select distinct nta.aluno, nta.disciplina, disciplina_equivalente = null,
       isnull(rec.nota_final_recuperacao, nta.nota) as NOTA_FINAL,   rec.nota_recuperacao, rec.nota_final_recuperacao,
   seperador = 'XXXXXXXXXXXXXXX',    dsc.nota as nota_historico
...
WHERE  
      isnull(rec.nota_final_recuperacao, nta.nota) <> dsc.nota and 
	  nta.curriculo_id = nta.curriculo_id_equ) as tab