with cte_aluno_prouni_fies as (
	...
	, cte_finalizado as (
	       ...
	       group by curriculo_aluno_id, etapa_ano_id, aluno_id, ingresso_nome, grade_id, qtd
			...
			select cte.*, aproveitamento = aprovado * 100.0 / qtd, case when (aprovado * 100.0 / qtd) < 75 then 'reprovado' else 'aprovado' end as status,
			...
			from cte_finalizado cte join academico_aluno      alu on (alu.id = cte.aluno_id)
		      ...
		      where alu.nome = 'ALINE ROSE MAGALHÃES BARBOSA BRAGA'MARCOS AMATOSHI TANAKA DIAS
