...
ALTER    procedure [dbo].[sp_corrigir_duplicidade_instituicao] @instituticao_nome varchar(max), @cidade_id int as 
					...
					BEGIN				
						...
						declare CUR_HIST cursor for 
							...
							open CUR_HIST 
								...
								while @@FETCH_STATUS = 0
									BEGIN
										EXEC SP_GERAR_LOG 'historicos_historico', @AUXILIAR, '~', 2136,@AUX, NULL, NULL
									fetch next from CUR_HIST into @AUXILIAR
									END
							close CUR_HIST 
						deallocate CUR_HIST
						...
						declare CUR_TITULACAO cursor for 
							...
							open CUR_TITULACAO 
								...
								while @@FETCH_STATUS = 0
									BEGIN
										EXEC SP_GERAR_LOG 'academico_pessoa_titulacao', @AUXILIAR, '~', 2136,@AUX, NULL, NULL
									fetch next from CUR_TITULACAO into @AUXILIAR
									END
							close CUR_TITULACAO 
						...
						declare CUR_DISINF cursor for 
							...
							open CUR_DISINF 
								...
								while @@FETCH_STATUS = 0
									BEGIN
										EXEC SP_GERAR_LOG 'curriculos_disciplinainformada', @AUXILIAR, '~', 2136,@AUX, NULL, NULL
									fetch next from CUR_DISINF into @AUXILIAR
									END
							close CUR_DISINF 
						...
						EXEC SP_GERAR_LOG core_instituicao, @AUXILIAR, '-', 2136, NULL, NULL, NULL
						SET @AUXILIAR = 0
						--  **** GERAR LOG FIM 
						delete ins
