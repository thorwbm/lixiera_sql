...
select * from [tmp_importacao_monitoria_2019] where aluno_nome = 'MARIA CLARA GRASSI MENDES MARINHO'

select len(nom_usu) from usuario where nom_usu like 'MARIA CLARA GRASSI MENDES%'
 ...
 -- insert into disciplina 
	  select distinct DISCIPLINA, disciplina_id, sigla =null   from tmp_importacao_monitoria_2019 imp	
	   where not exists (select * 
	                     from disciplina dis 
						where dis.des_dis COLLATE DATABASE_DEFAULT = imp.disciplina COLLATE DATABASE_DEFAULT and
						      dis.cod_dis_uni = imp.disciplina_id)	
	  order by 1
...
if exists(Select * from Tempdb..SysObjects Where Name Like '#tmp_import_final%') drop table #tmp_import_final
	...
	select tmp.curso, tmp.disciplina, tmp.aluno_nome,  tmp.aluno_cpf, tmp.etapa,  tmp.ano,max(isnull(tmp.nota,0)) nota
	 into #tmp_importacao
	  from tmp_importacao_monitoria_2019	tmp						  							  
	where 
		  tmp.situacao  in ( 'Aprovado', 'DISPENSADO', 'RESERVA') 
...
where not exists (select * from usuario_perfil upr where upr.cod_usu = acd.cod_usu and upr.cod_per = 2)
 ...
 --#################################################################################
 -- DIVERGENCIA DE NOMES 
 --#################################################################################
 /*
 select distinct tmp.aluno_cpf, tmp.aluno_nome, tmp.aluno_ra--, tmp.*
  from tmp_importacao_monitoria_2019 tmp left join usuario usu on (tmp.aluno_nome = usu.nom_usu)
where usu.cod_usu is null


select * from usuario
where   replace(replace(cpf_usu,'.',''),'-','') = '70077669606' --nom_usu like '%GRANATO%'


                            --Ana  Eliza Ignácio Mol 
update usuario set nom_usu = 'VINICIUS TEIXEIRA NOVAIS' where replace(replace(cpf_usu,'.',''),'-','') = '70077669606' 
 */