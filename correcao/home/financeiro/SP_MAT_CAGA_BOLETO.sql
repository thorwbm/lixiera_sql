...
select distinct
  ...
  from 
       academico_aluno alu   join VW_FNC_PARCELA_MAT_REM        par on (par.ra collate database_default = alu.ra collate database_default)  
...
--------------------------------------------------------------------------------------------------------------------  
 ...
 insert into 
    ...
    select 
           criado_em = @DATAEXECUCAO, atualizado_em = @DATAEXECUCAO, saldo = 0.0, criado_por = 11717,   
...
insert into 
       contratos_parcela (criado_em, atualizado_em, descricao, data_vencimento, valor, mes_competencia, ano_competencia, contrato_id, criado_por, tipo_id, atualizado_por)  
...
insert into   
    contratos_desconto ( descricao, porcentagem, valor, parcela_id, tipo_id, criado_em, criado_por, atualizado_em, atualizado_por)  
select   
