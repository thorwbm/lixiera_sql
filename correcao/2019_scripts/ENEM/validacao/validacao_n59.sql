-- exec SP_VALIDACAO_N59 '20191108'
CREATE or ALTER PROCEDURE sp_validacao_n59  @schema nvarchar(50) as 
declare @sql1 nvarchar(max)
declare @sql2 nvarchar(max)

--***** 1 - VERIFICAR SE AS COMPETENCIAS QUE ESTAO NA N59 BATEM COM O VALOR DA CORRECAO (PRIMEIRA CORRECAO)
set @sql1 = 
N'select ine.redacao_id, errro = 1
 ...
 where ine.lote_id in (select id from inep_lote where nome like ' + char(39) + '%' +  @schema + '%' + char(39) + ') and
       NOT exists(select 1 from [' + @schema + '].[correcoes_REDACAO] RED 
                   where ine.redacao_id = RED.id and 
				         isnull(ine.nu_nota_comp1_final,-5) = isnull(RED.nota_competencia1,-5) and 
						 isnull(ine.nu_nota_comp2_FINAL,-5) = isnull(RED.nota_competencia2,-5) and 
						 isnull(ine.nu_nota_comp3_FINAL,-5) = isnull(RED.nota_competencia3,-5) and 
						 isnull(ine.nu_nota_comp4_FINAL,-5) = isnull(RED.nota_competencia4,-5) and 
						 isnull(ine.nu_nota_comp5_FINAL,-5) = isnull(RED.nota_competencia5,-5) and 
...
from [dbo].[inep_n59] ine with(nolock) LEFT JOIN [' + @schema + '].[correcoes_redacao] red WITH(NOLOCK) ON (red.id = ine.redacao_id )
  WHERE ine.lote_id in (select id from inep_lote where nome like ' + char(39) + '%' +  @schema + '%' + char(39) + ') and 
		...
		ine.nu_nota_comp1_final <> red.nota_competencia1 or  
		ine.nu_nota_comp2_final <> red.nota_competencia2 or  
		ine.nu_nota_comp3_final <> red.nota_competencia3 or  
		ine.nu_nota_comp4_final <> red.nota_competencia4 or  
		ine.nu_nota_comp5_final <> red.nota_competencia5)  '
