...
/****** Object:  StoredProcedure [dbo].[sp_inserir_analise]    Script Date: 10/12/2018 21:31:03 ******/
SET ANSI_NULLS ON
GO
...
IF (@ERRO = '') 
	BEGIN 
		...
		/**/			where not exists(select top 1 1 from correcoes_filaauditoria AUD WITH (NOLOCK) WHERE AUD.co_barra_redacao = TEM.CO_BARRA_REDACAO)					
		/**/			set @VAI_PARA_AUDITORIA = 0

		/**/			select @VAI_PARA_AUDITORIA = 1 from correcoes_filaauditoria aud WHERE AUD.co_barra_redacao = @CODBARRA
					

					PRINT @VAI_PARA_AUDITORIA
			...
			/* EQUIDISTANCIA */
				...
				IF ((SELECT COUNT(COR1.ID)
                                    FROM CORRECOES_CORRECAO COR1 WITH (NOLOCK) JOIN CORRECOES_CORRECAO COR2 WITH (NOLOCK) ON (COR1.co_barra_redacao = COR2.co_barra_redacao AND 
                                                                                                                              COR1.id_tipo_correcao = 1 AND COR2.id_tipo_correcao = 2)
					                                            JOIN CORRECOES_CORRECAO COR3 WITH (NOLOCK) ON (COR3.co_barra_redacao = COR2.co_barra_redacao AND 
					        ...
					        COR1.CO_BARRA_REDACAO = @CODBARRA )> 0 )
				BEGIN
				...
				IF(@VAI_PARA_AUDITORIA = 0)
					BEGIN
						IF(EXISTS(SELECT 1 FROM CORE_FEATURE
							...
							END
						ELSE IF(EXISTS(SELECT 1 FROM CORE_FEATURE WHERE CODIGO = 'INSERE_NA_FILA_4_AUTOMATICO' AND FLAG_LIGADO = 1)AND
							     @CONCLUSAO > 2 and	
								 @TIPO_GRAVACAO = 3)  
							BEGIN
