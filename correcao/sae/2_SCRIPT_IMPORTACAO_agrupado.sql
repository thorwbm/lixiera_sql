...
select * from tbavaliacao where id_avaliacao >= 1461
        select distinct 
          ...
          from 
               VW_SAE_PARAMENTRO_IMPORTACAO where grade_nome = '2ª série'

			   select * from VW_SAE_PARAMENTRO_IMPORTACAO
              Desafio SAE 1 série - 1º Bimestre de 2020
   ################################################################## */
--    select * from tbcurso where id_curso = 120
-- select * from tbdisciplina where id_disciplina = 1699 
       declare @avaliacao_id int = 1466
   declare @AVALIACAO_EXT_ID INT = 74
    declare @ID_DISCIPLINA    int = 1695
    declare @ID_CURSO         int = 120
    declare @ID_PERIODO       int = 104
    declare @DATA_APLICACAO   datetime = '2020-05-07'
    declare @ID_INSTITUICAO   int = 22 
    declare @NR_RESPONDENTES  int = 3322 
...
INSERT INTO TbAvaliacaoAplicacao (id_avaliacao, ID_CURSO, ID_PERIODO, ID_DISCIPLINA,             
 ...
 SELECT DISTINCT  AVA.id_avaliacao, ID_CURSO = @ID_CURSO, ID_PERIODO = @ID_PERIODO , 
   ...
   FROM SAE_APPLICATION_APPLICATION APP JOIN SAE_USUARIO      USU ON (APP.user_id = USU.id)
                                        JOIN tbavaliacao AVA ON (AVA.id_avaliacao = @avaliacao_id)
   ...
   WHERE APP.exam_id = @AVALIACAO_EXT_ID and 
...
CREATE OR ALTER VIEW VW_SAE_ITEM AS
	    ...
	    from 
			...
			select top 10 * from VW_SAE_AVALIACAO where exa_avaliacao_id = 1461
*******************************************************************************************/