create or alter view VW_MOODLE_CURSO_DISCIPLINA AS 
select distinct cur.id as curso_id, cur.nome as curso_nome, 
	   ...
	   linha = cur.nome + ' - ' + dis.nome + ' - ' + gra.nome + ' - ' +
...
from academico_turmadisciplina tds
				 ...
				 join academico_etapaano       eta on (eta.id = gra.etapa_id)
where cat.id = 1 -- categoria = teorica

GO
----------------------------------------------------------------------------------------------------

select * from VW_MOODLE_CURSO_DISCIPLINA
where moodle_id is not null 
  and inicio_vigencia >= '2020-07-27'