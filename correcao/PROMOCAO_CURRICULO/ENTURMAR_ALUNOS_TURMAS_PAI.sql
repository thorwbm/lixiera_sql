...
select aluno_nome, aluno_id, curriculo_aluno_id, curso_nome, curriculo_nome, curriculo_aluno_status_nome
...
declare CUR_ENT cursor for 
	...
	--------------------------------------------------
	open CUR_ENT 
		fetch next from CUR_ENT into @CURRICULO_ALUNO_ID, @ALUNO_ID, @ALUNO_NOME, @SERIE,@DISCIPLINA_ID, 
			                         @DISCIPLINA_NOME, @TURMA_ORIGEM, @TURMA_DESTINO, @curso_nome,
		                             @TURMADISCIPLINA_ID
...
SELECT ALUNO_ID = @ALUNO_ID, TURMA_DISCIPLINA_ID = @TURMADISCIPLINA_ID, CRIADO_EM = GETDATE(), CRIADO_POR = 11717, ATUALIZADO_EM = GETDATE(), ATUALIZADO_POR = 11717, 
	   ...
	   WHERE NOT EXISTS (SELECT 1 FROM academico_turmadisciplinaaluno 
				...
				--				and 
			 --(select isnull(turx.maximo_vagas,0) - count(tdsx.id) 
			 -- from academico_turma turx join academico_turmadisciplina      tdsx on (turx.id = tdsx.turma_id)
				--				   left join academico_turmadisciplinaaluno tdax on (tdsx.id = tdax.turma_disciplina_id)
			 -- where tdsx.id = @TURMADISCIPLINA_ID  			        
			 --group by tdsx.id, isnull(turx.maximo_vagas,0)) > 0
...
select top 100 * from academico_turma
...
SELECT ALUNO_ID = tem.ALUNO_ID, TURMA_DISCIPLINA_ID = tem.turmadisciplina_id, CRIADO_EM = GETDATE(), CRIADO_POR = 11717, 
  ...
  FROM #temp_enturmacao tem 
WHERE NOT EXISTS (SELECT 1 FROM academico_turmadisciplinaaluno 
	                      WHERE ALUNO_ID = tem.ALUNO_ID AND 
						        TURMA_DISCIPLINA_ID = tem.turmadisciplina_id AND 
								CURRICULO_ALUNO_ID = tem.CURRICULO_ALUNO_ID) 
			 ...
			 SELECT isnull(turx.maximo_vagas,0),  count(tdsx.id) 
			  from academico_turma turx join academico_turmadisciplina      tdsx on (turx.id = tdsx.turma_id)
								   left join academico_turmadisciplinaaluno tdax on (tdsx.id = tdax.turma_disciplina_id)
			  where tdsx.id = 	 19077       
			 group by tdsx.id,turx.maximo_vagas



			 SELECT * FROM vw_acd_curso_turma_disciplina_grade WHERE turma_disciplina_id =  19077 AND vw_acd_curso_turma_disciplina_grade.etapa_ano = 2020

			 SELECT * FROM academico_turma WHERE id = 5754




			 SELECT distinct * -- ctd.curriculo_nome,  ctd.turma_nome, ctd.grade_nome
			   FROM VW_ACD_CURRICULO_TURMA_DISCIPLINA ctd join TMP_RENOVACAO_MATRICULA_2020 ren on (ren.curriculo = ctd.curriculo_nome)
			   where ctd.etapa_ano = 2020 and ctd.etapa_periodo = 2 and turma_disciplina_max_vaga is null 