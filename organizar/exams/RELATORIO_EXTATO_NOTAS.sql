...
with	cte_agendado as (
			select col.id as prova_id, col.name as prova_nome, 
				   ...
				   dateadd(hour, -3, etw.START_TIME) AS EXA_TWD_INICIO,dateadd(hour, -3, etw.END_TIME) AS EXA_TWD_TERMINO, etw.MAX_DURATION AS EXA_TWD_DURACAO,
				   ...
				   ans.value as nota, ans.updated_at as data_termino
			from exam_exam exa with(nolock) join exam_collection         col with(nolock) on (col.id = exa.collection_id)
										    ...
										    join exam_timewindow         etw with(nolock) on (exa.id = etw.exam_id)
)
...
select distinct age.prova_id, age.prova_nome, age.exame_id, age.exame_nome, age.EXA_TWD_INICIO, age.EXA_TWD_TERMINO, age.EXA_TWD_DURACAO, age.escola_nome, 
