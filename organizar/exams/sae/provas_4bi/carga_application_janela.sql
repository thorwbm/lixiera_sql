...
select distinct  started_at = null, finished_at = null, exa.exam_id, usu.id as user_id, should_update_answers = 0, timeout = null, forced_status = null, 
	   ...
	   into #temp_carga
  from  auth_user usu  join VW_COLLECTION_EXAM_4BI  exa on (exa.COLLECTION_GRADE = json_value(usu.extra, '$.hierarchy.grade.name'))
					   join tmp_imp_4bi_agendamento_blk blk on (blk.escolaNome = json_value(usu.extra, '$.hierarchy.unity.name') and 
									                            blk.serie = json_value(usu.extra, '$.hierarchy.grade.name'))
...
select * from #temp_carga car join exam_exam exa on (car.exam_id = exa.id) where start_time is null 
...
select * from application_answer where application_id = 5259262
