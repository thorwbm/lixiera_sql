...
into #temp_carga
from exam_collection exc join tmp_imp_escola_extra_2dia tmp on (ltrim(rtrim(left(exc.name,charindex( '-',exc.name)-1))) = tmp.avaliacao_diagnostica and 
...
where charindex( '-',exc.name) > 0 AND 
	...
	--  select tmp.*, exa.name
	  delete car 
	  from #temp_carga car join tmp_imp_escola_extra_2dia tmp on (tmp.nome_escola_ava = car.nome_escola and tmp.avaliacao_diagnostica = car.serie and tmp.lingua_espanhol = 'bloquear') 
	  ...
	  where exa.name like '%Língua Espanhola%'
	  	...
	  	--  select tmp.*, exa.name 
		  delete car 
		  from #temp_carga car join tmp_imp_escola_extra_2dia tmp on (tmp.nome_escola_ava = car.nome_escola and tmp.avaliacao_diagnostica = car.serie and tmp.lingua_ingles = 'bloquear') 
	                                join exam_exam exa on (exa.id = car.exam_id)
