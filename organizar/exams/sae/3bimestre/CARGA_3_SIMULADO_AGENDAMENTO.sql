...
SELECT TOP 50 * FROM TMP_IMP_ESCOLA_AGENDAMENTO ORDER BY ID DESC
...
into #temp_carga
from exam_collection exc join TMP_IMP_ESCOLA_AGENDAMENTO tmp on (LEFT(EXC.NAME,22) = 'Desafio SAE Teens 3°BI'   and 
                                                                 reverse( ltrim(rtrim(left(reverse(exc.name),charindex( '-',reverse(exc.name))-1)))) = TMP.SERIE)
...
where charindex( '-',exc.name) > 0 AND 
      XXX.id IS NULL  and 
	  BLK.ESCOLA_NOME IS NULL AND 
      exc.name like 'Desafio SAE Teens 3°BI%' 
	  -----------------------------------------
	 SELECT * FROM #temp_carga TMP JOIN TMP_IMP_ESCOLA_AGENDAMENTO IEA ON (TMP.ESCOLA_NOME = IEA.ESCOLA_NOME AND 
	 ...
	 DELETE TMP FROM #temp_carga TMP JOIN TMP_IMP_ESCOLA_AGENDAMENTO IEA ON (TMP.ESCOLA_NOME = IEA.ESCOLA_NOME AND 
	  ...
	  select distinct ESCOLA_NOME, serie, collection_nome, EXAME_NOME from #temp_carga 
