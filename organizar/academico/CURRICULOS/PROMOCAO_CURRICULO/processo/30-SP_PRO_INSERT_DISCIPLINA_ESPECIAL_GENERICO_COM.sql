...
/*****************************************************************************************************************  
*                                      SP_PRO_INSERT_DISCIPLINA_ESPECIAL_GENERICO_COM                            *  
*                                                                                                                *  
...
-- EXEC SP_PRO_INSERT_DISCIPLINA_ESPECIAL_GENERICO_COM 36360, 'INTEGRAÇÃO CURRICULAR', 'M069A03A202T'
-- EXEC SP_PRO_INSERT_DISCIPLINA_ESPECIAL_GENERICO_COM 36475, 'TREINAMENTO DE HABILIDADES', NULL

create or alter procedure [dbo].[SP_PRO_INSERT_DISCIPLINA_ESPECIAL_GENERICO_COM]   
       @CURRICULO_ALUNO_ID INT, @DISCIPLINA VARCHAR(200), @TURMA_PAI  VARCHAR(200) AS   


SET @TURMA_PAI =  ISNULL(@TURMA_PAI, '')  
...
select distinct 
       @turma_NOME = tur.nome, @aluno_id = tda.aluno_id,  
...
----------------------------------------------------------------------  
   select DISTINCT CGD.disciplina_ID  
     from VW_ACD_CURRIULO_CURSO_GRADE_DISCIPLINA CGD JOIN ACADEMICO_TURMA                  TUR ON (TUR.grade_id = CGD.grade_id)  
                                                     join VW_PRO_DISCIPLINA_NAO_CURSADA    ent on (ent.curriculo_aluno_id = @CURRICULO_ALUNO_ID and  
                                                                                                   ent.disciplina_id      = CGD.disciplina_id)  
 ...
 open CUR_INS_DIS   
  ...
  while @@FETCH_STATUS = 0  
   BEGIN  
    ...
    select @TURMADISCIPLINA_ID =  turma_disciplina_id 
	  from (select top 1 ctd.turma_disciplina_id, (ctd.maximo_vagas - count(tda.id)) as qtd   
    ...
    IF(ISNULL(@TURMADISCIPLINA_ID,0) > 0)   
		BEGIN  
			...
			SELECT ALUNO_ID = @ALUNO_ID, TURMA_DISCIPLINA_ID = @TURMADISCIPLINA_ID, CRIADO_EM = GETDATE(), CRIADO_POR = 11717, 
					ATUALIZADO_EM = GETDATE(), ATUALIZADO_POR = 11717, EXIGENCIA_MATRICULA_DISCIPLINA_ID = 2, 
					TIPO_MATRICULA_ID = 1, STATUS_MATRICULA_DISCIPLINA_ID = 14, FECHADO_EM = NULL, TENTAR_FECHAMENTO = 0, 
