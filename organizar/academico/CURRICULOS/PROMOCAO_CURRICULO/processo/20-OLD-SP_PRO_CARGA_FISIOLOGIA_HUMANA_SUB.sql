CREATE or alter PROCEDURE SP_PRO_CARGA_FISIOLOGIA_HUMANA_SUB AS 
...
declare CUR_PAI cursor for 
	select distinct  pai --, filha, filha_max_carga--, subfilha, subfilha_max_carga 
...
--------------------------------------------------------------------------------------------------
	        ...
	        declare CUR_FILHA cursor for 
				select distinct FILHA, FILHA_max_carga --, filha, filha_max_carga--, subfilha, subfilha_max_carga 
				from VW_PRO_CARGA_FISIOLOGIA_HUMANA 
				  WHERE PAI = @pai
				order by 1
				open CUR_FILHA 
					...
					while @@FETCH_STATUS = 0
						BEGIN
							...
							SET @CONT = (select COUNT(tda.id) 
							              from VW_ACD_CURRICULO_TURMA_DISCIPLINA tds left join academico_turmadisciplinaaluno tda on (tds.turma_disciplina_id = tda.turma_disciplina_id)										  
										  where (tds.disciplina_nome = 'FISIOLOGIA HUMANA II' and
								...
								BEGIN
									...
									SELECT ALUNO_ID = dag.aluno_id, TURMA_DISCIPLINA_ID = tds.turma_disciplina_id , CRIADO_EM = GETDATE(), 
                                      ...
                                      from #tmp_aux dag join VW_ACD_CURRICULO_TURMA_DISCIPLINA tds on (tds.disciplina_id = dag.disciplina_id and
                                                                                                         tds.turma_nome = @FILHA)
								                     left join academico_turmadisciplinaaluno xxx on (xxx.aluno_id = dag.aluno_id and 
													                                                  xxx.turma_disciplina_id = dag.TURMA_DISCIPLINA_ID )
									 WHERE xxx.id is null and
									       dag.TURMA_DISCIPLINA_ALUNO_ID = @TURMA_DISCIPLINA_ALUNO_ID
