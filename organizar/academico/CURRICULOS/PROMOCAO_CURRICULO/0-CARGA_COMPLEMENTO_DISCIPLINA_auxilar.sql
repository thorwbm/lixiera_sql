...
declare @curriculo_ori_id int = (select ID from curriculos_curriculo where nome = 'MEDICINA 2020/12-1 (1-2020)')
...
select distinct cap.aluno_id, cap.curriculo_aluno_id, dnc.disciplina_id, tur.id as turma_destino_id, cap.aluno_nome, cap.curriculo_id, 
		...
		into #temp_importacao
		 ...
		 where blk.aluno_id is null and
		       --cap.curriculo_id = @curriculo_ori_id and --
			   cap.curriculo_nome in ( 'MEDICINA 2020/12-1 (1-2020)') and 
			   cap.aluno_nome = 'ANDRÉ RIETRA DIAS COUTO' and 
			   cap.curriculo_aluno_status_id = 13   
         order by  dnc.disciplina_id,cap.curriculo_id, cap.aluno_nome
		 

		 select * from VW_PRO_DISCIPLINA_NAO_CURSADA where curriculo_aluno_id = 38469 and grade_id = 6986

		 select * from vw_Curriculo_aluno_pessoa where aluno_nome = 'ANDRÉ RIETRA DIAS COUTO'
