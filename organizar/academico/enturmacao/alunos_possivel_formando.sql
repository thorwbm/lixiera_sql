...
create view vw_possivel_formando_analitico_2020 as 
  ...
  from cte_aluno_disciplina cad left join curriculos_disciplinaconcluida dsc on (cad.curriculo_aluno_id = dsc.curriculo_aluno_id and 
	...
	,	cte_turmadisciplinaaluno_cursando as (
			select distinct tda.curriculo_aluno_id, tda.aluno_id, tds.disciplina_id 
			 ...
			 where tda.status_matricula_disciplina_id = 1 --and aluno_id =41318
)
	...
	,	cte_alunos_disciplina_1 as (
		   select cad.*, dis.nome as disciplina_nome, 
		   isnull (tda.situacao_tda, cad.situacao) as situacao_tda, 
		   eqv.disciplina 
...
select ald.aluno_id, ald.aluno_nome, ald.curriculo_aluno_id, ald.disciplina_id, ald.gradeDisciplina_id, ald.curriculo_nome, 
	   ...
	   situacao_final = case when situacao_tda = 'nao cursada' and equ.grade_disciplina_id is not null then 'cursando'
	                         when situacao_tda = 'concluido'   then 'concluido' 
							 when situacao_tda = 'cursando'     then 'cursando'
							 else 'nao cursada'  end
...
---select distinct aluno_id, aluno_nome from cte_final cte  where not exists(select 1 from cte_final ctex where cte.curriculo_aluno_id = ctex.curriculo_aluno_id and ctex.situacao_final = 'nao cursada' )
